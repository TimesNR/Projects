...
```sql
SELECT
Tabla.user_id,ROUND(COALESCE(Tabla.Confirmaciones / COUNT(Confirmations.action),0),2) AS confirmation_rate 
FROM
(SELECT Signups.user_id, COUNT(Confirmations.action) as Confirmaciones
FROM
(Signups LEFT JOIN Confirmations ON Signups.user_id = Confirmations.user_id AND Confirmations.action = "confirmed")
GROUP BY 
Signups.user_id) Tabla
LEFT JOIN
Confirmations 
ON  Tabla.user_id = Confirmations.user_id
GROUP BY Tabla.user_id
```
